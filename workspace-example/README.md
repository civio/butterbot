...
The smallest workspace Butterbot can do something with: one skill, no credentials
needed. `.env.example` points `BUTTERBOT_WORKSPACE` here, so a fresh clone can answer
a question and run a skill before you build a workspace of your own.

A workspace is just a directory the agent works in. Butterbot looks for two
